...
Notes for the next payload, learned the hard way:
  ...
  * A push made through the Claude GitHub connector did not reliably raise the
    ...
    CONFIRMED AGAIN 2026-09-04: arming the slot through the connector (246879d)
    produced no run at all after 3+ minutes. For a change that only touches a
    SMALL file (ws-sync-layer.js and friends), skip the slot entirely and push
    the file straight through the connector — that is what c58b01a did for the
    honey seed percentage. The slot is only worth arming for index.html, which
    is too large for the connector's file API.
